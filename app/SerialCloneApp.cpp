//
// SerialCloneApp.cpp  : Defines the entry point for the application.
//
...
SERIALCLONE_LIST_ITEM   g_IoList;
...
VOID SerialCloneOutputText(LPCTSTR Format, ...)
{
    ...
    OutputDebugString(_T("SerialClone: "));
...
//      status.
//
DWORD SerialCloneEnumerateDevices(HWND hDlg)
{
    HWND                                hList;
    DWORD                               lastError;
    LONG                                ii = 0;
    ...
    if (hList == NULL)
    {
        ...
        SerialCloneOutputText(_T("GetDlgItem failed, GetLastError() = %d"), lastError);
        return lastError;
    }
    ...
    hDeviceInfo = SetupDiGetClassDevs(
                    (LPGUID)&GUID_DEVINTERFACE_SERIALCLONE,
                    NULL,
                    NULL,
    ...
    {
        lastError = GetLastError();
        SerialCloneOutputText(_T("SetupDiGetClassDevs failed, GetLastError() = %d"), lastError);
        return lastError;
    }
         ...
         ++ii)
    {
                ...
                NULL))
        {
            ...
            {
                SerialCloneOutputText(_T("Error: couldn't get interface detail, (%d)"), GetLastError());
        ...
        {
            SerialCloneOutputText(_T("Error: Buffer allocation failed"));
            continue;
        }
        ...
        if (!SetupDiGetDeviceInterfaceDetail(
                ...
                NULL))
        {
            SerialCloneOutputText(_T("Error: SetupDiGetDeviceInterfaceDetail failed (%d)"), GetLastError());
            free(deviceDetail);
            continue;
        }
    ...
    if (ii == 0)
    {
        SerialCloneOutputText(_T("No devices found"));
    }
...
//      Handle to device.
//
HANDLE SerialCloneOpenDevice(HWND hDlg)
{
    HWND    hList;
    HANDLE  hDev;
    ...
    {
        SerialCloneOutputText(_T("Error: CreateFile failed for device %s (%d)\n"), path, GetLastError());
        return INVALID_HANDLE_VALUE;
    }

    SerialCloneOutputText(_T("Opened device %s"), path);
...
//      status.
//
LRESULT CALLBACK SerialCloneMainDlgProc(
    HWND    hDlg,
    UINT    uMsg,
    WPARAM  wParam,
    LPARAM  lParam
    )
{
    ...
    case WM_INITDIALOG:
        ...
        notificationFilter.dbcc_classguid = GUID_DEVINTERFACE_SERIALCLONE;
        g_hInterfaceNotification = RegisterDeviceNotification(hDlg, &notificationFilter, 0);

        // Populate the list box
        SerialCloneEnumerateDevices(hDlg);
    ...
    case WM_COMMAND:
        ...
        case IDC_OPEN_BUTTON:
            ...
            g_hDevice = SerialCloneOpenDevice(hDlg);
            if (g_hDevice == INVALID_HANDLE_VALUE)
            {
                SerialCloneOutputText(_T("Failed to open device, all options will be unavailable"));
                return 0;
            }
        ...
        case IDC_CLOSE_BUTTON:
            ...
            {
                SerialCloneOutputText(_T("Failed to close device"));
                return 0;
            }

            SerialCloneOutputText(_T("Device closed"));
                ...
                )
{
    ...
    // start dialog box
    int retVal = (int)DialogBox(hInstance, _T("SERIALCLONEAPP"), NULL, (DLGPROC)SerialCloneMainDlgProc);
